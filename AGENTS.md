...
- **TSD × Dolpago research:** Canon `.omx/plans/TSD_DaxAlgo_Dolpago_Requirements_v1.md`. Progress: `TSD_DaxAlgo_Dolpago_Implementation_Checklist.md`. **Handoff unit = saved finding** (code: `ResearchAnalysisReferenceV1`; do not say “Reference A/B” in user copy): `.omx/plans/research-strategy-reference-spine.md`. **Lane map + Nautilus:** `.omx/plans/research-strategy-backtest-reference-map.md`. Plans index: `.omx/plans/README.md`. Report `Spec ID → user action → code → verification → remaining`. Never invent alternate R-number schemes.
