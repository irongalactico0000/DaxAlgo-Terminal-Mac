...
- **TSD × Dolpago research:** Canon `.omx/plans/TSD_DaxAlgo_Dolpago_Requirements_v1.md`. Progress: `TSD_DaxAlgo_Dolpago_Implementation_Checklist.md`. **Handoff = saved finding** (not “Reference A/B”): `.omx/plans/research-strategy-reference-spine.md`. **Interaction design:** `.omx/plans/task-to-interface-map.md`, `.omx/plans/interaction-states-wireframes.md`, `.omx/plans/acceptance-journey-research-to-validate.md`. **Lane map + Nautilus:** `.omx/plans/research-strategy-backtest-reference-map.md`. Plans index: `.omx/plans/README.md`. Report `Spec ID → user action → code → verification → remaining`. Never invent alternate R-number schemes.
